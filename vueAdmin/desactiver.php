...
if(isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    if($recupUser->rowCount() > 0) {
        ...
        $desactiverUser = $bdd->prepare('UPDATE utilisateur SET isActive = 0 WHERE id_Utilisateur = ?');
        // Préparer une requête pour mettre à jour le champ isActive de l'utilisateur à 0
        $desactiverUser->execute(array($getid));
        // Exécuter la requête en remplaçant le paramètre par la valeur de l'identifiant
